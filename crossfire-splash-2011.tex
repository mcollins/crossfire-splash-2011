%-----------------------------------------------------------------------------
%
%               Template for sigplanconf LaTeX Class
%
% Name:         sigplanconf-template.tex
%
% Purpose:      A template for sigplanconf.cls, which is a LaTeX 2e class
%               file for SIGPLAN conference proceedings.
%
% Author:       Paul C. Anagnostopoulos
%               Windfall Software
%               978 371-2316
%               paul@windfall.com
%
% Created:      15 February 2005
%
%-----------------------------------------------------------------------------


\documentclass[preprint]{sigplanconf}

% The following \documentclass options may be useful:
%
% 10pt          To set in 10-point type instead of 9-point.
% 11pt          To set in 11-point type instead of 9-point.
% authoryear    To obtain author/year citation style instead of numeric.

\usepackage{amsmath}
\usepackage{graphicx}
\usepackage{subfigure}
\usepackage{multirow}
\usepackage{rotating}
\usepackage{array}
\usepackage{algorithmic}
\usepackage{algorithm}
\usepackage{textcomp}
\usepackage{listings}
\usepackage{hyperref}
\begin{document}

\conferenceinfo{SPLASH '11}{date, City.}
\copyrightyear{2011}
\copyrightdata{[to be supplied]}

\titlebanner{banner above paper title}        % These are ignored unless
\preprintfooter{short description of paper}   % 'preprint' option specified.

\title{Crossfire - Multiprocess, Cross-Browser, Open Web Debugging Protocol}
\subtitle{Subtitle Text, if any}

\authorinfo{Michael G. Collins}
           {IBM Research - Almaden}
           {mgcollin@us.ibm.com}
\authorinfo{John J. Barton}
           {IBM Research - Almaden}
           {johnjbarton@johnjbarton.com}

\maketitle

\input{abstract.tex}

\category{CR-number}{subcategory}{third-level}

\terms
term1, term2

\keywords
keyword1, keyword2

\input{introduction.tex}

\input{jjb.tex}
\input{background.tex}
\input{jjb.tex}
\input{protocol.tex}
\input{implementation.tex}
\input{discussion.tex}
\input{relatedwork.tex}
\input{futurework.tex}
\input{conclusion.tex}

\appendix
\section{Appendix Title}

This is the text of the appendix, if you need one. I don't

\acks

Acknowledgments, if needed.

% We recommend abbrvnat bibliography style.

\bibliographystyle{abbrvnat}
\bibliography{bibliography}

% The bibliography should be embedded for final submission.

\begin{thebibliography}{}
\softraggedright

%\bibitem[Smith et~al.(2009)Smith, Jones]{smith02}
%P. Q. Smith, and X. Y. Jones. ...reference text...
\bibitem[Eclipse JSDT]{EclipseJSDT}
http://wiki.eclipse.org/JSDT/Debug
\bibitem[VitekDynamicJS2010]{VitekDynamicJS2010}
Richards, Lebresne, Burg, Vitek, An Analysis of the Dynamic Behavior of JavaScript Programs. PLDI
\end{thebibliography}

\end{document}
